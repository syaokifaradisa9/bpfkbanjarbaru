                        ...
                        @for ($i = 0; $i < ceil(count($orders)/2); $i++)
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="{{ route('fasyankes.order.'.$order_type.'.certificates.print', ['id' => $order_id, 'alkes_order_id' => $orders[$i]->id]) }}">
                        ...
                        @for ($i = 0; $i < ceil(count($orders)/2); $i++)
                            ...
                            @if ($i + ceil(count($orders)/2) < count($orders))
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="{{ route('fasyankes.order.'.$order_type.'.certificates.print', ['id' => $order_id, 'alkes_order_id' => $orders[$index]->id]) }}">
                                            <i class="fas fa-file-alt mr-1"></i> File
                                        </a>
                                    </td>
                                </tr>
